...
namespace MonitorJudicial
{
    public partial class _Default : Page
    {
        ...
        public void PorcentajeCasos()
        {
            ...
            abandono = porcentajes.ContainsKey("ABANDONO") ? porcentajes["ABANDONO"] : 0.0;
            abstencionTrmiteParteJuez = porcentajes.ContainsKey("ABSTENCIÓN DE TRÁMITE POR PARTE DEL JUEZ") ? porcentajes["ABSTENCIÓN DE TRÁMITE POR PARTE DEL JUEZ"] : 0.0;
            adjudicacion = porcentajes.ContainsKey("ADJUDICACIÓN ") ? porcentajes["ADJUDICACIÓN "] : 0.0;
            alegatos = porcentajes.ContainsKey("ALEGATOS") ? porcentajes["ALEGATOS"] : 0.0;
            apelacion = porcentajes.ContainsKey("APELACIÓN ") ? porcentajes["APELACIÓN "] : 0.0;
            avaluoBienes = porcentajes.ContainsKey("AVALUÓ DE BIENES") ? porcentajes["AVALUÓ DE BIENES"] : 0.0;
            ...
            liquidacion = porcentajes.ContainsKey("LIQUIDACIÓN ") ? porcentajes["LIQUIDACIÓN "] : 0.0;
            mandamientoEjecucion = porcentajes.ContainsKey("MANDAMIENTO DE EJECUCIÓN ") ? porcentajes["MANDAMIENTO DE EJECUCIÓN "] : 0.0;
            ninguno = porcentajes.ContainsKey("NINGUNO") ? porcentajes["NINGUNO"] : 0.0;
            noContestaDemanda = porcentajes.ContainsKey("NO CONTESTA DEMANDA") ? porcentajes["NO CONTESTA DEMANDA"] : 0.0;
            otros = porcentajes.ContainsKey("OTROS") ? porcentajes["OTROS"] : 0.0;
            presentacionDemanda = porcentajes.ContainsKey("PRESENTACIÓN DEMANDA") ? porcentajes["PRESENTACIÓN DEMANDA"] : 0.0;
            prueba = porcentajes.ContainsKey("PRUEBA") ? porcentajes["PRUEBA"] : 0.0;
            remate = porcentajes.ContainsKey("REMATE") ? porcentajes["REMATE"] : 0.0;
            sentencia = porcentajes.ContainsKey("SENTENCIA") ? porcentajes["SENTENCIA"] : 0.0;
            suspendidoAcuerdo = porcentajes.ContainsKey("SUSPENDIDO POR ACUERDO") ? porcentajes["SUSPENDIDO POR ACUERDO"] : 0.0;
            terminadoAcuerdoPagoObligaciones = porcentajes.ContainsKey("TERMINADO POR ACUERDO O PAGO DE OBLIGACIONES") ? porcentajes["TERMINADO POR ACUERDO O PAGO DE OBLIGACIONES"] : 0.0;
            aprehencionVehicular = porcentajes.ContainsKey("APREHENCION VEHICULAR") ? porcentajes["APREHENCION VEHICULAR"] : 0.0;
            audiencia = porcentajes.ContainsKey("AUDIENCIA") ? porcentajes["AUDIENCIA"] : 0.0;
            razonNoPago = porcentajes.ContainsKey("RAZÓN DE NO PAGO") ? porcentajes["RAZÓN DE NO PAGO"] : 0.0;
            audienciaEjecucion = porcentajes.ContainsKey("AUDIENCIA EJECUCIÓN") ? porcentajes["AUDIENCIA EJECUCIÓN"] : 0.0;
